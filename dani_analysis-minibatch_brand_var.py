...
import socket
import warnings
from gp_models import DirichletGPModel, MeanFieldDecoupledModel
...
if __name__ == '__main__':
    ...
    latent_dim_res = 2048  # res50 res101
    ...
    for num_inducing in [900]: #[1500]: #[300,600,900,1200,1500,1800,2100]: #[300, 900, 1200]:
        ...
        for latent_dim in [20,25,30]: #[3,5,10,15,20]:
            para_str = "inducing_"+str(num_inducing)+"dim_"+str(latent_dim)
            wandb.init(
                # project="fashion_gp_project",
                project="fashion_brands",
                notes = 'resnet101 & MeanFieldDecoupledModel'
            )
            ...
            # model training
            model.train()
            initial_lengthscales = model.covar_module.base_kernel.lengthscale.detach().cpu().numpy()
            optimization_trace = []
                ...
                for j, (x_batch, y_batch, batch_index) in enumerate(train_loader):
                    ...
                    with torch.no_grad():
                        wandb.log({
                            ...
                            'full_loss': wandb.Histogram(full_loss.detach().cpu().numpy()),
                            'hyper/lengthscale': wandb.Histogram(
                                model.covar_module.base_kernel.lengthscale.view(-1).detach().cpu().numpy()),
                            'hyper/outputscale': wandb.Histogram(model.covar_module.outputscale.detach().cpu().numpy()),
                            'hyper/second_noise': wandb.Histogram(likelihood.second_noise.view(-1).detach().cpu().numpy()),
