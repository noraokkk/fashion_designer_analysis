...
if __name__=="__main__":
    ...
    if args.backbone_type == 'cnn':
        model = resnet_model(args.num_classes, backbone=args.backbone)
    elif args.backbone_type == 'Vit':
        model = Vit(args.num_classes,args.backbone)
    else:
        print('invalid backbone name.')
        exit(0)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        ################### Train #################
        acc_train, loss_train = run_epoch(args, model, train_loader, optimizer, train=True,
                                          warmup_scheduler=scheduler_warmup)
        loss_train_all.append(loss_train)

        ################### Valid #################
        acc_val, loss_val = run_epoch(args, model, val_loader, optimizer, train=False,
                                      warmup_scheduler=scheduler_warmup)
        loss_val_all.append(loss_val)

        ################### Test #################
        if test_loader is not None:
            acc_test, loss_test = run_epoch(args, model, test_loader, optimizer, train=False,
                                            warmup_scheduler=scheduler_warmup)
        ...
        best_valid, best_test = metrics_logger.evaluate_AUC_genearl(acc_train, acc_val, acc_test, epoch, 0, model,
                                                                    loss_val, loss_test, args)
        print(args.model_name)
